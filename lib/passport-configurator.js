  ...
  switch (authType) {
    ...
    case 'local':
      ...
      function(username, password, done) {
        ...
        console.log('Fetching query', JSON.stringify(query));
        self.userModel.findOne(query, function(err, user) {
          ...
          if (user) {
            console.log('There is a user! Hooray');
            var u = user.toJSON();
            delete u.password;
            var userProfile = {
              provider: 'local',
            ...
            var login = function(creds) {
              self.userModel.login(creds,
                function(err, accessToken) {
                  console.log('self.userModel.login response callback function');
                  console.log(err, accessToken);
                  if (err) {
                    console.log('There was unfortunately an error');
                    console.log(err);
                  ...
                  if (accessToken && user.emailVerified) {
                    console.log('Access token and email is verified, looking good');
                    userProfile.accessToken = accessToken;
                    done(null, userProfile, {accessToken: accessToken});
                  } else {
                    console.log('Failure, went to the else');
                    done(null, false, {message: g.f('Failed to create token.')});
                  }
                });
            };
            if (options.setAccessToken) {
              console.log('Attempting a log in and setAccessToken with', options.usernameField);
              ...
              console.log('Oh no! This shouldnt be called!')
              return user.hasPassword(password, function(err, ok) {
                console.log('return user.hasPassword');
                console.log(err, ok);
            ...
            console.log('findOne there was no user!', errorMsg);
            done(null, false, {message: errorMsg});
          }
        });
      }
  ...
  var defaultCallback = function(req, res, next) {
    ...
    passport.authenticate(name, _.defaults({session: session},
      ...
      if (!user) {
        ...
        if (!!options.json) {
          console.log('Returning authentication error');
        ...
        console.log('Redirecting to failure redirect');
      ...
      if (session) {
        ...
        req.logIn(user, function(err) {
          ...
          if (err) {
            console.log('Error in session login');
            console.log(err);
            return next(err);
          }
          if (info && info.accessToken) {
            console.log('Info and access token');
            if (!!options.json) {
              console.log('Success! Returning JSON with access token');
              return res.json({
                'access_token': info.accessToken.id,
                userId: user.id,
              });
            } else {
              console.log('Success! Returning JSON with cookie');
          ...
          console.log('Success! Going to the success redirect');
          return res.redirect(successRedirect(req));
        });
      } else {
        console.log('There is no session');
        if (info && info.accessToken) {
          console.log('Info and access are present');
          if (!!options.json) {
            console.log('Success! Returning JSON with access token');
            return res.json({
              'access_token': info.accessToken.id,
