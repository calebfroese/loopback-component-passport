...
module.exports = function(UserIdentity) {
  ...
  /**
   * Resolve Email for User Object
   * Checks whether or not the useProfileEmail is set and set the User object accordingly
   * keeps Legacy email
   * @parram {String} provider
   * @param {String} profileEmail
   * @param {String} generatedEmail
   * @param {Object} options
   */
  function resolveEmail (provider, profileEmail, generatedEmail, options) {
    if (provider === 'ldap') return profileEmail;
    var userProfileEmail = options.profileEmail;
    return userProfileEmail ? profileEmail : generatedEmail;
  }
    /**
   * Resolve Usrername for User Object
   * Checks whether or not the useProfileEmail is set and set the User object accordingly
   * @parram {String} provider
   * @param {String} profile
   * @param {Object} options
   */
  function resolveUsername (provider, profile, options) {
    var userNameField = options.userNameField;
    if (userNameField) return profile[userNameField];
    return provider + '.' + (profile.username || profile.id);
  }
  ...
  // Let's create a user for that
    ...
    var email = resolveEmail(provider, profileEmail, generatedEmail, options);
    var username = resolveUsername(provider, profile, options);
